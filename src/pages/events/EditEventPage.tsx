...
import { useNavigate, useParams } from 'react-router-dom'; // useParams for fetching event ID
...
interface EventData {
    ...
    capacity: number;
    status: string;
    isPrivate: boolean; // Include isPrivate in the type
}

const EditEventPage: React.FC = () => {
    const navigate = useNavigate();
    const { id: eventId } = useParams<{ id: string }>(); // Get event ID from URL params
    const { user } = useUser();
    ...
    const [errors, setErrors] = useState({
        ...
        price: '',
        capacity: '',
        general: '', // For handling general errors, like a 403 status code
    });
    ...
    useEffect(() => {
            ...
            try {
                ...
                if ('data' in response) {
                    ...
                    setEventData({
                        ...
                        status: eventInfo.status || 'active', // Default status if missing
                        isPrivate: eventInfo.isPrivate || false, // Default to false (public)
                    });
                }
            } catch (error) {
        ...
        fetchEventDetails(); // Call the effect without conditionally skipping it
    }, [eventId, user?.token]);
    ...
    const validateForm = () => {
        const newErrors = {
            ...
            price: '',
            capacity: '',
            general: '', // Reset general errors on validation
        };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const requestBody = {
            ...
            isPrivate: eventData.isPrivate, // Include isPrivate when updating
        };
