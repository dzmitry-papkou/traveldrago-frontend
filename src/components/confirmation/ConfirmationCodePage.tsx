...
const ConfirmationCodePage = () => {
  ...
  const { isLoading, errors, sendData } = useQuery<{ message: string }>({
    ...
    includeTokenInBody: !location.state?.isEmailUpdate, // Include token in body only for initial email confirmation
  });
  ...
  const handleConfirmCode = () => {
    ...
    const payload = location.state?.isEmailUpdate
      ? { verificationCode: code }  // Use "verificationCode" when verifying after an update
      : { confirmationCode: code };  // Use "confirmationCode" for initial confirmation
